...
* **ci:** prevent E2BIG error in release workflow ([714cd36](https://github.com/Androlax2/dato-builder/commit/714cd36a494c69334303f044cdf7aca68dce08d2))


### Code Refactoring

* **core:** overhaul CLI, API, config & builder modules ([#26](https://github.com/Androlax2/dato-builder/issues/26)) ([#27](https://github.com/Androlax2/dato-builder/issues/27)) ([#41](https://github.com/Androlax2/dato-builder/issues/41)) ([2fa1427](https://github.com/Androlax2/dato-builder/commit/2fa14270674ad8f827c2543004d9c0d4b0258be4)), closes [#28](https://github.com/Androlax2/dato-builder/issues/28) [#29](https://github.com/Androlax2/dato-builder/issues/29) [#44](https://github.com/Androlax2/dato-builder/issues/44)
...
- Introduce `MixedDependencies.integration.test.ts` to validate complex mixe…

* fix(release): update NPM_TOKEN to NODE_AUTH_TOKEN in release workflow
...
* feat: add new blocks for accessibility features, fare management, and event tracking

* fix: update .gitignore to include src/datocms directory

* fix: update spellcheck command to suppress progress and summary output
...
* fix: update .npmignore to include additional configuration and development files

* fix: remove outdated troubleshooting section from README.md

* fix: update package.json and package-lock.json to include @commander-js/extra-typings dependency

* fix: enhance release workflow to check and run integration tests based on changes

* feat: add integration tests on demand workflow triggered by comments and manual dispatch

* feat: enhance CI workflow with manual dispatch and integration change detection

* fix: update tsconfig.build.json to retain comments and adjust internal stripping

* fix: remove integration test requirement check from release workflow

* fix: remove detect-changes job from integration tests workflow and simplify triggers

* feat: add concurrency control to integration tests job in on-demand workflow
...
* feat: enhance configuration loading with environment override and logging

* feat: add environment property to DatoBuilderConfig for logging and behavior customization

* feat: add environment option to CommandBuilder for environment specification

* refactor: remove environment option from CommandBuilder and simplify configuration loading

* feat: set default environment to 'main' in configuration loading

* feat: include environment in DatoApi client initialization for ItemTypeBuilder and RunCommand

* feat: replace DatoBuilderConfig with ResolvedDatoBuilderConfig across multiple files

* feat: add environment handling in configuration and tests for DatoBuilderConfig

* feat: add integration tests and configuration for environment-specific builds

* feat: add environment configuration options to README and update examples

* test: clear mock calls and update environment handling in TestItemTypeBuilder tests
...
* fix: ensure inflection dependency is properly bundled
...
* fix: remove problematic npm ci flags in release workflow

  - Remove --prefer-offline and --no-audit flags that may cause dependency resolution
  issues
  - Ensures inflection dependency is properly installed during CI build process
...
* fix: optimize npm install performance in release workflow

  - Add --no-audit flag to npm ci for faster dependency installation
  - Security verification still handled by separate audit signatures step
...
* fix: update dependencies to version 30.0.5 and remove npm cache from release workflow

* fix: rebuild package with correct version during release

- Remove npm cache to prevent stale dependency/build issues
- Move build process to semantic-release prepare phase after version update
- Ensures published package has correct version and dependencies
...
* fix: ensure version replacement runs after package.json version update

  - Reorder semantic-release plugins to update version before build
  - Replace deprecated assert with 'with' syntax for JSON imports
  - Fixes CLI --version returning incorrect version number

* fix: use namespace import for inflection to resolve ES module
  compatibility

  - Change from 'import inflection from' to 'import * as inflection from'
  - Fixes 'Cannot read properties of undefined (reading 'singularize')' error
  - Resolves CommonJS/ES module import issue with inflection package

* chore: restore npm cache and maintain build-time version replacement

- Re-enable npm cache in CI for performance
- Keep build-time version replacement approach for cleaner package distribution
- Maintain prepublishOnly hook pattern for semantic-release compatibility
...
* fix: add 'unique' validator option to SlugBody type
